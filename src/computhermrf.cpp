...
#define ADDRESS_BIT_COUNT   16
#define PADDING_BIT_COUNT   4

#define ENABLE_DEBUG        0
#define DEBUG_PIN           14
#define DEBUG2_PIN          16
...
{
    bool result = false;
    /* Workaround: check only address bits! */
    /* Reson: some devices send 0xB padding instead of 0x7 */
    for (int i = 0; i < ADDRESS_BIT_COUNT; i++)
    {
        if (_buff[i] != _lastBuff[i])
        {
            for (int j = 0; j < ADDRESS_BIT_COUNT + PADDING_BIT_COUNT; j++)
            {
                _lastBuff[j] = _buff[j];
            }
...
void ICACHE_RAM_ATTR ComputhermRF::_handler()
{
    ...
    if (!_avail)
    {
        ...
        { // Rising edge, only stop could be detected
            if (diffMs >= _STOP_MIN)
            {
#if ENABLE_DEBUG
                digitalWrite(DEBUG_PIN, LOW);
